...
try
    ...
    [x,y]=meshgrid(-texsize:texsize + PixPerCyc, 1);
    grating=gray + inc*cos(fr*x);
    
    [x2,y2]=meshgrid(-texsize/2:texsize/2 + PixPerCyc, 1);
    m=cos(fr*x2);
    grating2=gray + inc*m;
    ...
    % Recompute PixPerCyc, this time without the ceil() operation from above.
    % Otherwise we will get wrong drift speed due to rounding!
    PixPerCyc=1/CycPerPix; % pixels/cycle
    
    % Translate requested speed of the grating (in cycles per second)
    % into a shift value in "pixels per frame", assuming given
    % waitduration: This is the amount of pixels to shift our "aperture" at
    % each redraw:
    shiftperframe= cyclespersecond * PixPerCyc * waitduration;
