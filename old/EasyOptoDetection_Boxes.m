...
try
    ...
    prompt = {'Mouse Name', 'Exp Nr', 'Date', 'Laser power', 'Task Name', 'Color', 'Setup'};
    def = {'Name', '1', datestr(datenum(date), 'yyyymmdd'), 'NaN', 'EasyOptoDetection', 'Red', 'Box'};
    ...
    % choose box number
    Boxnrs = {'1', '2', '3', '4', '5', '6'};
    Boxchoice = menu('Choose the Setup Nr',Boxnrs);
    Boxnr = Boxnrs{Boxchoice};
    Log.Setup = [Log.Setup Boxnr];
    ...
    while ~stopaftertrial
        ...
        % don't have the option to have opto in boxes yet
    ...
    for f = 1:length(Log_fieldnames)
        ...
        if isempty(FieldData)
            fprintf('skipping this field %s', char(Log_fieldnames(f)))
        else
