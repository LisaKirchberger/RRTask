function moving_Lines(StarType,NumStars)
% Sample script starsrgb v1.32
%
% Creates a smoothly moving starfield display.  The display 
% is in direct colour mode.
%
% Usage:-   starsrgb(StarType,NumStars)
%
...
% Clear globals
%
clear global Num x y Rad MinRad Spd tm
%
return
%-----------------------------------------------------
...
if tm.FrmCnt > 0
    str = sprintf('starsrgb v1.32 P:%s Tim:%02d:%02d:%02d Frm:%d Av:%-.2fHz Drp:%d',...
        cogstd('spriority'),...
	...
	str = sprintf('starsrgb v1.32 P:%s Tim:%02d:%02d:%02d Frm:%d Drp:%d',...
        cogstd('spriority'),...
        fix(S/3600),mod(fix(S/60),60),mod(fix(S),60),...
        tm.FrmCnt,...
        tm.Drp);
